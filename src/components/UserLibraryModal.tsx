...
export default function UserLibraryModal({
    ...
    useEffect(() => {
        ...
        api.get(`/admin/users/${userId}/library`)
            .then((res) => {
                    ...
                    <>
                        ...
                        <ModalHeader className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                {userName && (
                                    <span className="text-sm font-normal text-default-500">({userName})</span>
                                )}
                            </div>
                        </ModalHeader>
                            ...
                            {isLoading ? (
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-3 border-b border-default-200">
                                        {[
                                            { label: "Colors", count: data.counts.colors, icon: "mdi:palette" },
                                            { label: "Gradients", count: data.counts.gradients, icon: "mdi:gradient-horizontal" },
                                            { label: "Palettes", count: data.counts.palettes, icon: "mdi:view-grid" },
                                        ].map((item) => (
                                            <div key={item.label} className="flex flex-col items-center py-4 gap-1">
                                                <Icon icon={item.icon} width={20} className="text-[#7C3AED]" />
                        ...
                        <Tab key="colors" title="Colors">
                                            <div className="flex flex-col gap-2 px-4 pb-4 pt-2">
                                                {data.colors.length === 0 ? (
                                                    <p className="text-center text-sm text-default-500 py-8">No colors saved</p>
                                                ) : data.colors.map((c) => (
                                                    <div
                                                        key={c.colors_id}
                                                    ...
                                                    >
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-sm font-semibold text-foreground">{c.color_name}</p>
                                                            <p className="text-xs text-default-500 font-mono">{c.hex_code}</p>
                                                        </div>
                                                        <span className="ml-auto text-xs text-default-400">#{c.colors_id}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </Tab>
                                        ...
                                        <Tab key="gradients" title="Gradients">
                                            <div className="flex flex-col gap-2 px-4 pb-4 pt-2">
                                                {data.gradients.length === 0 ? (
                                                    <p className="text-center text-sm text-default-500 py-8">No gradients saved</p>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            />
                                                            <div className="min-w-0">
                                                                <p className="text-sm font-semibold text-foreground">{g.gradient_name}</p>
                                                                <p className="text-xs text-default-500 font-mono">{g.gradient_code}</p>
                                                            </div>
                                                            <span className="ml-auto text-xs text-default-400">#{g.gradients_id}</span>
                                                        </div>
                                                    );
                                                })}
                                            </div>
                                        ...
                                        <Tab key="palettes" title="Palettes">
                                            <div className="flex flex-col gap-2 px-4 pb-4 pt-2">
                                                {data.palettes.length === 0 ? (
                                                    <p className="text-center text-sm text-default-500 py-8">No palettes saved</p>
                                                ) : data.palettes.map((p) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="min-w-0">
                                                                <p className="text-xs text-default-500 font-mono break-all">{p.palette_code}</p>
                                                            </div>
                                                            <span className="ml-auto text-xs text-default-400">#{p.palettes_id}</span>
                                                        </div>
                                                    );
                                                })}
                                            </div>
